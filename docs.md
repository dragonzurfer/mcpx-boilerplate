...
- `web/`: HTML templates and static assets (Tailwind browser runtime `@tailwindcss/browser@4` + vanilla JS), including GIS-rendered Google sign-in button slots, the Career Copilot dossier-style tool UI with an all-path guided-selection map, course catalog cards + sidebar lesson explorer, admin course builder, resilient client-side loading states for home/courses/tools/pricing feeds (class + native `hidden` + inline display fallback), tolerant home-feed response parsing, parallel `/api/me` + page-data frontend bootstrap, centered post reading layout, and pricing value/savings positioning.
