...
- `templates/`: SSR templates (layout + page content, including tools + account + admin pages and shared navigation, plus post analytics and admin tools controls; post detail uses a centered reading column).
...
- `assets/`: Tailwind browser runtime styles (`@tailwindcss/browser@4`) + vanilla JS app, including resilient loader visibility toggles that use both class and native `hidden` state for home/tools/pricing, home-feed parsing that accepts common API envelope variants, parallel frontend bootstrap (`/api/me` hydration + page fetches), pricing-plan benefit/savings messaging, and a full focus-path explorer in Career Copilot guided selection.
