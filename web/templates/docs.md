...
- `course.html`: course learning layout with three reading rails: left roadmap (`course-modules-sidebar`), wide centered lesson content viewer (`course-body`), and right sticky heading index (`course-toc` inside `course-toc-shell`), plus a top promo slot container (`course-promo-slot`) rendered above lesson metadata; the markdown body no longer uses `max-w-none` so lesson content remains centered inside the card.
