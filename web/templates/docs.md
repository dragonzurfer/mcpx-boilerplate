...
- `course.html`: course learning layout with sidebar roadmap (`course-modules-sidebar`), narrow centered Medium-style lesson content viewer (`course-body`), and a top promo slot container (`course-promo-slot`) rendered above lesson metadata; the markdown body no longer uses `max-w-none` so lesson content remains centered inside the card.
